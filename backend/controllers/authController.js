...
const updateUserProfile = async (req, res) => {
    try {
        ...
        res.json({ id: updatedUser.id, name: updatedUser.name, email: updatedUser.email, university: updatedUser.university, address: updatedUser.address, token: generateToken(updatedUser) });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
